...
const UploadIcon: React.FC<{ className?: string }> = ({ className = 'text-zinc-500' }) => (
    <svg xmlns="http://www.w3.org/2000/svg" className={`h-12 w-12 mx-auto mb-2 ${className}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1}>
...
const ImageUploader = forwardRef<HTMLImageElement, ImageUploaderProps>(({ id, label, onFileSelect, imageUrl, isDropZone = false, onProductDrop, persistedOrbPosition, showDebugButton, onDebugClick, isTouchHovering = false, touchOrbPosition = null }, ref) => {
  ...
  useEffect(() => {
    ...
    // When imageUrl changes (new upload, reset, or parent changes),
    // we must reset the loaded state to show the skeleton for the new image.
  ...
  const handleClick = (event: React.MouseEvent<HTMLDivElement>) => {
    // If it's a drop zone and the image is loaded, a click should place the product.
    if (isDropZone && onProductDrop && isImageLoaded) {
      handlePlacement(event.clientX, event.clientY, event.currentTarget);
    } else {
      // Otherwise (it's a simple uploader, or a dropzone that's still loading), open the file dialog to replace/upload.
  ...
  const handleDragOver = useCallback((event: React.DragEvent<HTMLDivElement>) => {
      ...
      // File drags are always allowed, show file drag feedback
      if (isFileDrag) {
      ...
      // Product drags are only allowed on a loaded dropzone
      if (isDropZone && onProductDrop && !isFileDrag && isImageLoaded) {
          setIsDraggingOver(true);
      ...
      // If it's a product drag over a loading image, we do nothing, so no feedback is shown.
  }, [isDropZone, onProductDrop, isImageLoaded]);
  ...
  const handleDrop = useCallback((event: React.DragEvent<HTMLDivElement>) => {
      ...
      // Priority: Handle file drops first, as they are explicit user actions to upload.
      ...
      // If no files were dropped, it might be an internal product drag.
      else if (isDropZone && onProductDrop && isImageLoaded) {
          handlePlacement(event.clientX, event.clientY, event.currentTarget);
      }
  }, [isDropZone, onProductDrop, onFileSelect, handlePlacement, isImageLoaded]);
  
  const isProductHovering = isDraggingOver || isTouchHovering;
  // The orb should only be visible when dragging a product over a fully loaded image.
  const showOrb = isProductHovering && isImageLoaded && !isFileDraggingOver;
  
  // This just combines the two possible position sources. The visibility is handled by `showOrb`.
  ...
  const uploaderClasses = `w-full aspect-video bg-zinc-100 border-2 border-dashed rounded-lg flex items-center justify-center transition-all duration-300 relative overflow-hidden ${
      isFileDraggingOver ? 'border-blue-500 bg-blue-50 is-file-dragging-over'
    : showOrb ? 'border-blue-500 bg-blue-50 is-dragging-over'
      ...
      >
        ...
        {imageUrl ? (
          <>
            ...
            {!isImageLoaded && (
                <div className="absolute inset-0 w-full h-full bg-zinc-200 animate-pulse" role="status" aria-label="Loading image..."></div>
            )}
            <img 
              ref={imgRef}
              src={imageUrl} 
            ...
            <div className="uploader-overlay">
                <div className="text-center text-white p-4">
                    ...
                    <p className="font-bold text-lg">Drop to replace</p>
                </div>
            </div>
            <div 
                className={`drop-orb ${showOrb ? 'is-visible' : ''}`}
                style={{ 
                    // Position is always updated, visibility is controlled by the `is-visible` class.
